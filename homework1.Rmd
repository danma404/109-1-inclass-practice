...
```{r ans12}
a=ymd_hms("2020-10-6 21:00:00", tz="America/New_York")
with_tz(a,tz="Asia/Taipei")->b
with_tz(a,tz="Europe/Paris")->c
list(
  jason=list(as.Date(b),
             OlsonNames(b))
...
```{r ans21}
y=factor(chNumbers,levels(y)<-ch1to9)
as.integer(factor(y))->y 
sumChNumbers=sum(y)
```
...
```{r ans4}
list(location1, meta)=list(library(location),library(meta))
```
