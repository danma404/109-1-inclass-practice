...
```{r ans12}
NYTime=ymd_hms("2020-10-6 21:00:00", tz="America/New_York")
with_tz(NYTime,tz="Asia/Taipei")->TaipeiTime
with_tz(NYTime,tz="Europe/Paris")->ParisTime
d=list(Jason=list(
  Date=date(TaipeiTime),
  StartTime=paste0(hour(TaipeiTime),":",minute(TaipeiTime),":",second(TaipeiTime)),
  TimeZone=tz(TaipeiTime) ),
  Louis=list(
  Date=date(ParisTime),
  StartTime=paste0(hour(ParisTime),":",minute(ParisTime),":",second(ParisTime)),
  TimeZone=tz(ParisTime) ),
  Allen=list(
  Date=date(NYTime),
  StartTime=paste0(hour(NYTime),":",minute(NYTime),":",second(NYTime)),
  TimeZone=tz(NYTime) )
  )
Game5Time=c(Jason=c(Date=paste(d$Jason$Date),StartTime=paste(d$Jason$StartTime),TimeZone=paste(d$Jason$TimeZone)),
            Louis=c(Date=paste(d$Louis$Date),StartTime=paste(d$Louis$StartTime),TimeZone=paste(d$Louis$TimeZone)),
            Allen=c(Date=paste(d$Allen$Date),StartTime=paste(d$Allen$StartTime),TimeZone=paste(d$Allen$TimeZone)))
Game5Time
...
```{r ans13}
f=paste0(Game5Time[[1]],"/",Game5Time[[2]])
g=paste0(Game5Time[[4]],"/",Game5Time[[5]])
h=paste0(Game5Time[[7]],"/",Game5Time[[8]])
G5TimeParsed=c(f,g,h)
G5TimeParsed
```
...
```{r ans15}
TpeReply=TaipeiTime+lubridate::minutes(15)
ParisLouis=format(TpeReply)
ParisLouis
```
...
```{r ans21}
chNumbers=factor(chNumbers,levels(chNumbers)<-ch1to9)
as.integer(factor(chNumbers))->y 
sumChNumbers=sum(y)
sumChNumbers
```
...
```{r ans22}
x=factor(chNumbersWith0,levels = ch0to9)
as.integer(x)->x
as.integer(randomLength)->z
sumChNumbersWith0=sum(x)-z
sumChNumbersWith0
```
...
```{r ans31}
escapeText="我們要用成對的\"或\'將文字包起來，R才會知道它是字串。\n字串在輸入時，若要cat時在螢幕看起來有換行則必需輸入\\n，其中\\稱為Escape key，它讓後面字元跳脫了它在電腦的原始含意。"
cat(escapeText)
```
...
```{r ans32}
dearStudent=paste0("Dear ",paste(student$name),":\n\n  貼心提醒您，作業上傳截止時間是",paste(student$deadline),"。\n\n敬祝學安\n\n你的貼心學伴巴西利bot silly")
cat(dearStudent)
```
